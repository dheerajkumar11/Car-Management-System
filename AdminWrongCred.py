...
def AdminLoginpage():
    ...
    mycursor.execute("SELECT * FROM UserCredentials WHERE Username = '"+ userid_entry.get()+"' AND user_password = '"+password_entry.get()+"' AND Department = 'admin';")
    result = mycursor.fetchone()

    if result == None :
        subprocess.Popen(["python", "AdminWrongCred.py"])
        window.destroy()
...
window = tkinter.Tk()
window.title("AutoDealer Insight Admin Login")
window.geometry("1360x1000")

bg_image = Image.open("adminlogin123.png")
...
user_info_frame = tkinter.LabelFrame(frame, text='Admin Login',font=("Arial", 20 , 'bold'), bg="#DFF8FC", padx=20, pady=20)
...
btnSignIn = tkinter.Button(buttons_frame, width=10, text="Sign-In", font=("Comic Sans MS", 20 , 'bold'), command=AdminLoginpage)
